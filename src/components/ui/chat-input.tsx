...
import { ArrowUpIcon, FolderOpenIcon } from "lucide-react";
...
interface ChatInputToggleProps {
	className?: string;
	useDeepContentSearch: boolean;
	onUseDeepContentSearchChange: (checked: boolean) => void;
}

function ChatInputToggle({
	className,
	useDeepContentSearch,
	onUseDeepContentSearchChange,
}: ChatInputToggleProps) {
	const { t } = useTranslation();
	const id = "deep-content-search-toggle";

	return (
		<div className={cn("flex items-center space-x-2", className)}>
			<Switch
				id={id}
				checked={useDeepContentSearch}
				onCheckedChange={onUseDeepContentSearchChange}
			/>
			<Label htmlFor={id} className="text-sm">
				{t("components.chatInput.deepContentSearch")}
			</Label>
		</div>
	);
}

ChatInputToggle.displayName = "ChatInputToggle";

export { ChatInput, ChatInputTextArea, ChatInputSubmit, ChatInputToggle };
