...
export const getToolDisplayName = (toolName: string, t: TFunction) => {
	if (toolName === "getRelevantContent") {
		return t("components.toolInvocation.relevantSources");
	}
	if (toolName === "getPageContent") {
		return t("components.toolInvocation.pageContent");
	}
	return t("components.toolInvocation.toolResults");
};
